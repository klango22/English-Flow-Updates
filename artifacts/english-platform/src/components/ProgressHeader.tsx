import { Flame, Star, Zap, RotateCcw, Trash2, ChevronRight } from 'lucide-react';
...
export function ProgressHeader({ state, onMasterReset, onWeekReset, onNextDay }: Props) {
  ...
  const xpToday = dayProg.xp;
  const xpGoal = 200;
  const xpPercent = Math.min(100, (xpToday / xpGoal) * 100);
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-border bg-card/90 backdrop-blur-sm">
      ...
      {/* Lock notice */}
      {!canAdvance && (
        <div className="text-center text-xs text-muted-foreground py-1 bg-muted/30 border-t border-border/50">
