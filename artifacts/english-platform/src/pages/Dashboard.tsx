...
import { getLevelBadge, getLevelProfile } from "@/lib/adaptiveEngine";
      ...
      <div className="max-w-5xl mx-auto px-4 py-2.5 flex items-center justify-between flex-wrap gap-2">
        <div className="flex items-center gap-2">
          ...
          <p className="text-xs text-foreground">
            <span className="font-semibold">Progress not saved.</span> Log in to
            sync your XP, streak, and exercises across devices — it only takes a
            second.
          </p>
        </div>
        <div className="flex items-center gap-2">
        ...
        <div className="max-w-5xl mx-auto px-4 py-2.5 flex items-center justify-between flex-wrap gap-2">
          <div className="flex items-center gap-2">
            ...
            <p className="text-xs font-semibold text-amber-300">
              Catch-up Mode — completing{" "}
              <span className="font-bold">
                Week {behindWeek}, Day {behindDay}
              </span>
            </p>
          </div>
          <button
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-amber-400/80">
              ...
              (90 min + exercises) to unlock today's content.
            </p>
          </div>
        </div>
        <button
...
export default function Dashboard() {
  ...
  const lesson = getLesson(state.currentWeek, state.currentDay, state.totalXP);
  ...
  const tabs: {
    ...
    { id: "lessons", label: "Lessons", icon: <BookOpen size={15} /> },
    {
      ...
      count: grammarTasks.length,
    },
    {
      id: "listening",
      label: "Listening",
      icon: <Headphones size={15} />,
      count: listeningTasks.length,
    },
    {
      id: "speaking",
      label: "Speaking",
      icon: <MessageSquare size={15} />,
      count: speakingTasks.length,
    },
    {
      id: "vocabulary",
      label: "Vocab",
  ...
  const bannerWeek = isViewingCatchUpDay
    ? state.currentWeek
    : calendarSync.calendarWeek;
  const bannerDay = isViewingCatchUpDay
    ? state.currentDay
    : calendarSync.calendarDay;
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      {!authLoading && !isAuthenticated && <GuestBanner onLogin={login} />}
      ...
      >
        <div className="max-w-5xl mx-auto px-4 py-4">
          <div className="flex items-center justify-between flex-wrap gap-2">
            <div>
              <p
                className={`text-xs font-bold uppercase tracking-widest ${isViewingCatchUpDay ? "text-amber-400" : "text-primary"}`}
              >
                Week {bannerWeek} · Day {bannerDay}
                {isViewingCatchUpDay && (
                  <span className="ml-2 normal-case text-amber-500">
                    (catch-up)
                  </span>
                )}
              </p>
              ...
              <p className="text-sm text-muted-foreground">
                90-Minute Immersion Plan · 20 Tasks ·
                <span className="text-violet-400 font-semibold ml-1">
                  {getLevelBadge(state.totalXP)}
                </span>
              </p>
            </div>
            <div className="flex gap-4 text-center">
              ...
              <div>
                <p className="text-2xl font-bold text-foreground">
                  {minutesDone}
                  <span className="text-base text-muted-foreground">/90</span>
                </p>
          ...
          <div className="mt-3 grid grid-cols-2 gap-3">
            ...
            <div>
              <div className="flex justify-between text-xs text-muted-foreground mb-1">
                ...
                <span
                  className={
                    accuracy >= 80 ? "text-emerald-400" : "text-amber-400"
                  }
                >
                  {accuracy}%
                </span>
          ...
          <div className="flex gap-1 overflow-x-auto py-2 scrollbar-hide">
            {tabs.map((tab) => (
              ...
              >
                ...
                {tab.count !== undefined && (
                  <span
                    className={`text-xs px-1.5 py-0.5 rounded-full ${activeTab === tab.id ? "bg-white/20" : "bg-muted"}`}
                  >
                    {tab.count}
                  </span>
                )}
      ...
      <div className="max-w-5xl mx-auto px-4 py-6">
        ...
        {activeTab === "grammar" && (
          <div className="space-y-4">
            ...
            <p className="text-sm text-muted-foreground">
              Complete all grammar exercises to strengthen your understanding.
            </p>
            <div className="space-y-4">
        ...
        {activeTab === "listening" && (
          <div className="space-y-4">
            ...
            <p className="text-sm text-muted-foreground">
              Listen to each passage and answer the comprehension questions.
            </p>
            <div className="space-y-4">
        ...
        {activeTab === "speaking" && (
          <div className="space-y-4">
            ...
            <p className="text-sm text-muted-foreground">
              Practice your spoken English. Each task plays the prompt for you.
            </p>
            <div className="space-y-4">
        ...
        {activeTab === "vocabulary" && (
          <div className="space-y-4">
            ...
            <p className="text-sm text-muted-foreground">
              Tap any card to flip and reveal the definition. Use the speaker to
              hear pronunciation.
            </p>
